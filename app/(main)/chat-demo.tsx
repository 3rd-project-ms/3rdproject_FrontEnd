...
const HIDDEN_SCRIPT = [
  {
    ...
    sender: "ai",
    instant: true,
    text: "The night breeze is so nice, isn't it? Thank you for taking a walk with me. I really wanted to see you today.",
  },
  {
    time: 7.0,
    duration: 4.0,
    sender: "user",
    text: "Me too. The night view of the river is so beautiful today.",
    clearMissionIds: [1],
    userStart: 7.0,
    userEnd: 11.0,
    affinityDelta: 1,
  },
  {
    ...
    duration: 7.0,
    sender: "ai",
    text: "It really is... but honestly, I haven't been looking at the view at all. Can you guess what I've been looking at?",
  },
  {
    time: 18.0,
    duration: 4.0,
    sender: "user",
    text: "Me? You're making me blush. Since when did you look at me like that?",
    clearMissionIds: [2],
    userStart: 18.0,
    userEnd: 22.0,
    affinityDelta: 1,
  },
  {
    ...
    duration: 8.0,
    sender: "ai",
    text: "Well, I am friendly, but I don't give away free cookies to just anyone. I don't want to just be the cafe owner who makes your coffee anymore.",
  },
];
...
export default function ChatDemoScreen() {
  ...
  const {
    ...
    hint,
  } = getStageMissions(stageNum, "voice");
  ...
  const [micState, setMicState] = useState<"idle" | "recording" | "disabled">(
    "idle",
  );
  ...
  const userTextFadeAnim = useRef(new Animated.Value(1)).current;
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      ...
      if (duration > 0 && currentTime >= duration - 0.5) {
        ...
        setMicState("idle");
        clearInterval(interval);
        Animated.timing(userTextFadeAnim, {
          toValue: 0,
          duration: 1000,
          useNativeDriver: true,
        }).start();
      ...
      while (
        ...
        if (item.instant) {
          ...
          const onComplete =
            item.clearMissionIds?.length || (item as any).affinityDelta != null
              ? () => {
                  if (item.clearMissionIds?.length) {
                    setMissions((prev) =>
                      prev.map((m) =>
                        item.clearMissionIds!.includes(m.id)
                          ? { ...m, cleared: true }
                          : m,
                      ),
                    );
                  ...
                  if (delta != null) {
                    setAffinity((prev) =>
                      Math.min(100, Math.max(0, prev + delta)),
                    );
                    showDelta(delta);
                  }
                }
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.textDisplayArea}>
        ...
        <View style={styles.speechRow}>
          ...
          <Animated.Text
            style={[styles.speechTextUser, { opacity: Animated.multiply(userTextOpacity, userTextFadeAnim) }]}
          >
        ...
        <View style={styles.modalOverlay}>
          ...
          <View style={styles.modalCard}>
            ...
            >
              ...
              <Text
                style={{ fontSize: 17, fontWeight: "700", color: "#0B0B12" }}
              >
                힌트
              </Text>
            </View>
            ...
            <Text
              style={{ fontSize: 13, color: "#888888", textAlign: "center" }}
            >
              {hint.korean}
            </Text>
            <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  speechTextUser: { flex: 1, flexShrink: 1, fontSize: 14, color: "#555555", lineHeight: 20 },

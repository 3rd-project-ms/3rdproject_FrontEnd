...
import {
  ...
  SafeAreaView, ScrollView, Platform, Image,
} from 'react-native';
...
const DEMO_NICKNAME = 'Sims';
const DEMO_TOTAL = 3;
const DEMO_SCORE = 80;

export default function DemoMyPageScreen() {
  const router = useRouter();
  const [showLevelModal, setShowLevelModal] = useState(false);
  const [showEditModal, setShowEditModal] = useState(false);
  const [selectedCode, setSelectedCode] = useState('B1');
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.card}>
          <View style={styles.profileRow}>
            <Image
              source={require('../../assets/images/sims.png')}
              style={styles.avatar}
            />
            <View style={styles.profileInfo}>
              <Text style={styles.nickname}>{DEMO_NICKNAME}</Text>
            </View>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.statsGrid}>
            <View style={styles.statItem}>
              <Text style={styles.statValue}>{DEMO_TOTAL}회</Text>
              <Text style={styles.statLabel}>총 대화 횟수</Text>
            </View>
            <View style={[styles.statItem, styles.statItemRight]}>
              <Text style={styles.statValue}>{DEMO_SCORE}점</Text>
        ...
        <View style={styles.card}>
          <View style={styles.levelHeader}>
            ...
            <TouchableOpacity style={styles.levelChangeBtn} onPress={() => setShowLevelModal(true)}>
          ...
          <View style={styles.levelGrid}>
            {LEVELS.map((lv) => {
              ...
              return (
                ...
                >
                  <Text style={[styles.levelCellCode, isSelected && styles.levelCellCodeActive]}>{lv.code}</Text>
                  <Text style={[styles.levelCellLabel, isSelected && styles.levelCellLabelActive]}>{lv.label}</Text>
                  <Text style={[styles.levelCellDesc, isSelected && styles.levelCellDescActive]}>{lv.desc}</Text>
                </View>
              );
            })}
          </View>
        ...
        <View style={styles.card}>
          <TouchableOpacity style={[styles.menuItem, styles.menuItemBorder]} onPress={() => router.push('/report/demo-mode-report' as any)}>
          ...
          <TouchableOpacity style={styles.menuItem} onPress={() => router.push('/review/demo-review' as any)}>
      ...
      {showLevelModal && (
        <View style={styles.modalOverlay}>
          <TouchableOpacity style={StyleSheet.absoluteFillObject} onPress={() => setShowLevelModal(false)} />
          <View style={styles.modalCard}>
            ...
            <TouchableOpacity style={styles.modalCancelBtn} onPress={() => setShowLevelModal(false)}>
      ...
      {showEditModal && (
        <View style={styles.modalOverlay}>
          <TouchableOpacity style={StyleSheet.absoluteFillObject} onPress={() => setShowEditModal(false)} />
          <View style={styles.modalCard}>
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    paddingHorizontal: 16, paddingTop: Platform.OS === 'android' ? 45 : 12,
    paddingBottom: 12, backgroundColor: '#F8F8F8',
  },
  backBtn: { padding: 4 },
  ...
  sectionTitle: { fontSize: 13, fontWeight: '600', color: '#AEAEB2', marginBottom: 0 },
  profileRow: { flexDirection: 'row', alignItems: 'center', gap: 14 },
  avatar: { width: 52, height: 52, borderRadius: 26, backgroundColor: '#F6A3A6', alignItems: 'center', justifyContent: 'center' },
  profileInfo: { flex: 1 },
  ...
  levelHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 },
  levelGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
  levelCell: { width: '47.5%', borderRadius: 12, borderWidth: 1.5, borderColor: '#F0F0F0', backgroundColor: '#FAFAFA', padding: 14 },
  ...
  modalTitle: { fontSize: 16, fontWeight: '700', color: '#0B0B12', marginBottom: 16, textAlign: 'center' },
  ...
  modalCancelBtn: { marginTop: 16, paddingVertical: 14, backgroundColor: '#F0F0F5', borderRadius: 12, alignItems: 'center' },

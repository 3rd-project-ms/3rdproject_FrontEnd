...
// ─── 메인 스크린 ──────────────────────────────────────────────────────────
export default function DemoStageScreen() {
  const router = useRouter();

  const charName    = '시엔나';
  const charRole    = '카페 사장님';
  const characterId = 'CH_03_F';

  const bgImage = STAGE_BACKGROUNDS[charRole] ?? STAGE_BACKGROUNDS['서핑 강사'];

  const [affinity, setAffinity]           = useState<number>(80);
  const [activeStage, setActiveStage]     = useState<AnyStage | null>(null);
  const [stageList, setStageList]         = useState<AnyStage[]>(() => buildStageList(80));
  const [isStagesLoaded, setIsStagesLoaded] = useState(true);
              ...
              {activeStage?.isLocked ? (
                ...
                <View style={styles.modalBtnRow}>
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      setActiveStage(null);
                      router.push({ pathname: '/(main)/chat-text-demo' as any });
                    }}
                  >
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      setActiveStage(null);
                      router.push({ pathname: '/(main)/chat-demo', params: { stage: 'stage1' } });
                    }}
                  >
